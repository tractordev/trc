// this is a fork of the fn package of duplex and should be pushed back to that project
package golang
...
func ArgsTo(fntyp reflect.Type, args []any) ([]reflect.Value, error) {
	...
	for idx, param := range args {
		...
		default:
			...
			} else if fntyp.In(idx).Kind() == reflect.Int && reflect.TypeOf(param).Kind() == reflect.String {
				param, _ = strconv.Atoi(param.(string))
			}
